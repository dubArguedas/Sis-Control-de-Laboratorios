...
namespace SCLAB_API.Controllers
{
     ...
     *    - Registra la asistencia de un docente mediante c�digo QR
     *    - Valida usuario docente, m�quina, laboratorio y cronograma activo
     *    - Cambia el estado de m�quina y laboratorio a "ocupado"
     *    - Evita registros duplicados en el mismo horario
     * 
     * 2. PUT /api/AsistenciasDocente/{id}/observacion
     *    - Actualiza la observaci�n de una asistencia de docente
     * 
     * 3. GET /api/AsistenciasDocente/{id}
     *    - Obtiene los detalles completos de una asistencia espec�fica de docente
     * 
     * 4. GET /api/AsistenciasDocente/docente/{usuarioId}
     *    - Obtiene el historial de asistencias de un docente espec�fico
     * 
     ...
     *    - Cambia el estado de la m�quina a "disponible" (si no est� en mantenimiento)
     *    - Cambia el estado del laboratorio a "libre" si no hay m�s asistencias activas
     * 
     ...
     *    - Busca asistencias de ESTUDIANTES por d�a y rango horario
     * 
    ...
    public class AsistenciasDocenteController : ControllerBase
    {
        ...
        public async Task<IActionResult> FinalizarAsistenciaDocente(int id)
        {
            try
            {
                ...
                // Cambiar el estado de la m�quina a disponible (solo si no est� en mantenimiento)
                if (asistencia.Maquina != null && asistencia.Maquina.Estado.ToLower() != "mantenimiento")
                {
                    asistencia.Maquina.Estado = "libre";
                }

                // Verificar si hay m�s asistencias activas en el laboratorio
                ...
                // Si no hay m�s asistencias activas, cambiar el estado del laboratorio a libre
